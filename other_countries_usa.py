"""Create HOT-format files for USA-specific refreshes outside the main HOTOSM set."""
...
def parse_args():
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(description="Generate USA-specific HOT files.")
...
def find_rows(ids, temp_dir, osm_filepath, index):
    ...
    temp_file = os.path.abspath(os.path.join(temp_dir, f"temp_usa_{index}.db"))
...
def process_country(country, pred_pattern, temp_dir, index):
    """Load prediction rows for a country from parquet partitions."""
    temp_file = os.path.abspath(os.path.join(temp_dir, f"temp_country_usa_{index}.db"))
    conn = duckdb.connect(temp_file)
    try:
        query = f"""
        SELECT
...
def merge(countries, pred_pattern, original_osm_dir, temp_dir, output_dir, output_subdir):
    ...
    for index, country in enumerate(countries.keys()):
        predictions = process_country(country, pred_pattern, temp_dir, index)
        ...
        temp_file = os.path.abspath(os.path.join(temp_dir, f"temp_join_usa_{index}.db"))
        conn = duckdb.connect(temp_file)
        try:
...
def main(config_path):
    """Run workflow for creating USA-specific output files."""
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s - %(message)s")
    config = load_config(config_path)
    script_config = get_section(config, "other_countries_usa")
    ...
    temp_dir = os.path.abspath(os.path.join(output_dir, "temp_usa"))
    os.makedirs(temp_dir, exist_ok=True)

    pred_pattern = os.path.join(predictions_dir, "predictions_*.parquet")
    merge(
